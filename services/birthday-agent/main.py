"""
Envoltura HTTP (Flask) para el Birthday Agent (ADK). Este servicio es
privado en Cloud Run: solo el Agent Master (y, para descargar archivos, el
frontend) tienen permiso roles/run.invoker sobre él.

Endpoints:
    POST /invoke   { "user_id": "...", "session_id": "...", "message": "..." }
                   -> { "response": "texto final del agente" }
    GET  /files/<path:file_path>  -> descarga el PDF/JPG generado
    GET  /health
"""
...
def download_file(file_path: str):
    data, content_type = cards.download_bytes(file_path)
